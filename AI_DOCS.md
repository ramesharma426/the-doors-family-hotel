...
5. **Beer Zone** (`#beerzone`) — "The Beer Zone", right after Rock 'n' Roll Zone. Full-bleed fixed-background (parallax) kiosk photo (`assets/img/beer_zone.jpg`, `.live`'s pattern) — disabled on mobile via `background-attachment:scroll`, same as Live — plus a small gold-framed, slightly tilted video card (`.beerzone-clip`) of a draft pour looping at its own native 9:16 aspect ratio (not stretched to fill the section — that was tried first and cropped/zoomed too tightly to read), autoplay-on-scroll like the Video Reel clips (not eager like the Hero). "Grab A Pint" CTA (`tel:`).
