...
5. **Beer Zone** (`#beerzone`) — "The Beer Zone", right after Rock 'n' Roll Zone. Full-bleed background kiosk photo (`assets/img/beer_zone.jpg`) — fixed/parallax on desktop (`.live`'s pattern; `center top` position so the "BEER ZONE" sign near the top of the photo doesn't get cropped by a wide-short section), `background-size:contain` (no crop at all, static) on mobile — see the mobile-parallax note under §4 for why this section deliberately does **not** get the JS parallax treatment Live gets. Plus a small gold-framed, slightly tilted video card (`.beerzone-clip`) of a draft pour looping at its own native 9:16 aspect ratio (not stretched to fill the section — that was tried first and cropped/zoomed too tightly to read), autoplay-on-scroll like the Video Reel clips (not eager like the Hero). "Grab A Pint" CTA (`tel:`). Merch credit reads "Merch in partnership with @riff_volt" (not "Merch & memorabilia").
...
- **Why Beer Zone's photo doesn't get the mobile parallax treatment** — it was tried (same `.parallax-img` pattern as Live) and looked broken: `object-fit:cover` on a phone-width/tall-narrow box zooms in far enough that the "BEER ZONE" sign board — the photo's dominant subject — renders oversized and overlapping the badge/heading, confirmed by an actual mobile screenshot, not just computed-style checks (those had all looked fine, which is *why* this shipped once already without being visually caught — computed styles proved the mechanism worked, not that the crop looked right). Live's band photo tolerates the same crop fine since cropping into people/instruments just looks like a photo, not illegible text. Lesson: for any section whose photo has text/signage as the dominant subject, take an actual screenshot at a real mobile viewport before trusting a cover-crop treatment — do this *before* calling the work done, not after the user reports it's still broken.
