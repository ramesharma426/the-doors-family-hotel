...
| `assets/icons/` | `facebook.svg`, `instagram.svg`, `tiktok.svg`, `whatsapp.svg` — `fill="#efe6d3"`, used only by the sticky social rail (footer no longer duplicates these), always rendered at the same fixed CSS box size (`.social-btn img`/`.social-sticky .social-btn img`). `whatsapp`/`instagram`/`tiktok` use `viewBox="0 0 24 24"` with the artwork filling ~60-83% of that box on each axis. `facebook`'s artwork is a narrow "f" — its viewBox is intentionally cropped tight to the glyph's own bounding box (`viewBox="5.3 0.7 13.2 23.1"`, not `0 0 24 24`) so it fills a comparable ~72-75%; the original full-24×24 viewBox left so much dead padding around the thin glyph that it rendered visibly smaller than the other three at the same CSS size even though every icon's declared `width`/`height` attributes matched. If a new icon is ever added here, check its artwork's actual fill ratio within its viewBox (e.g. via `element.getBBox()`) before assuming matching `width`/`height` attributes alone guarantee visual parity. |
